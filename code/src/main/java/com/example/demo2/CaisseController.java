...
public class CaisseController implements Initializable{
    ...
    @FXML
    private TableView<PharmaCaisse.medicaments> affichageMedicaments;
    @FXML
    private TableColumn<PharmaCaisse.medicaments, Integer> id;
    @FXML
    private TableColumn<PharmaCaisse.medicaments, String> name;
    @FXML
    private TableColumn<PharmaCaisse.medicaments, Integer> qty;
    @FXML
    private TableColumn<PharmaCaisse.medicaments, Float> price; // Corrected the type of prix
    @FXML
    ...
    ObservableList<PharmaCaisse.medicaments> medicamentsList = FXCollections.observableArrayList();
    ...
    private void refreshTable() throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            Integer quantity = resultSet.getInt("quantite"); // Retrieve as String
            double price = resultSet.getDouble("prix_unitaire_achat");

            medicamentsList.add(new PharmaCaisse.medicaments(id, quantity, name, price));}
        affichageMedicaments.setItems(medicamentsList);}

    private void loadData() throws SQLException {
        DatabaseConnection connectNow = new DatabaseConnection();
        connection= connectNow.getConnection("PharmaGest", "postgres", "1234");
        refreshTable();
    ...
    private void setupRowClickListener() {
        affichageMedicaments.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
            if (newSelection != null) {
                PharmaCaisse.medicaments selectedMedicament = newSelection;
                String selectedNom = selectedMedicament.getName();
                Integer selectedQty = selectedMedicament.getQty();
                double selectedPrice = selectedMedicament.getPrice();
    ...
    public void addReceipt(ActionEvent event) {
        try {
            PharmaCaisse.medicaments selectedMedicament = affichageMedicaments.getSelectionModel().getSelectedItem();
            ...
            String receiptString = selectedNom + "  " + "                                 " + selectedPrice + "rs";
            receiptList.add(receiptString);

            for (int i = 0; i < quantityValue - 1; i++) {
                receiptList.add("                                                                    " + String.valueOf(selectedPrice));}
            receiptList.add("                                                              total: " + total + "rs");
            receiptTotal+= total;

            nomMed.add(selectedNom);
            prxMed.add(selectedPrice);
            ttlMed.add(total);
    ...
    public void printReceipt(ActionEvent event)throws SQLException{
        ...
        query = "INSERT INTO recu (total, vendeur, id_recu) VALUES (?, 'maeva',?)";
        preparedStatement = connection.prepareStatement(query);
        ...
        for (int i = 0; i < nomMed.size(); i++){
            ...
            int item4 = prxMed.get(i);
            try {
                query = "INSERT INTO recu_details (nom_medicament, prix_medicament, quantite_medicament, total, id_details) VALUES (?, ?, ?, ?,?)";
                preparedStatement = connection.prepareStatement(query);
                preparedStatement.setString(1, nomMed.get(i));
                preparedStatement.setInt(2, prxMed.get(i));
